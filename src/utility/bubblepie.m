function bubblepie(xlist,ylist,slist,graph_data,graph_labels,xlab,ylab,lab,islog,colors)
...
graph_max_size = 0.10; %0.25;
graph_min_size = 0.01;%0.05;
...
if lab
    for i = 1:size(graph_data,1)
        ...
        %colororder(colors)
        try
            piechart(d,repmat({''},length(d),1))
        catch
            pie(d,repmat({''},length(d),1))
        end
        cc = 1;
        for pp = 1 : 2 : numel(p)
            p(pp).FaceColor = colors{cc};
            cc = cc + 1;
        end
    ...
    for i = 1:size(graph_data,1)
        ...
        %colororder(colors)
        try
            p = piechart(d,repmat({''},length(d),1));
